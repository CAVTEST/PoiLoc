...
class ImageCalibrator(PerspectiveFields):
    ...
    def run(
        ...
        if focal_length is None and exif is not None:
            ...
            # print(f"focal_ratio:{focal_ratio}")
            if focal_ratio != 0:
        ...
        # print(f"roll_pitch:{roll_pitch}")
        hfov = 0
        if focal_length is None:
            vfov = calib["pred_vfov"].item()
            focal_length = h / 2 / np.tan(np.deg2rad(vfov) / 2)
            hfov = np.rad2deg(2 * np.arctan(w / (2 * focal_length)))
            # print(f"focal_length：{focal_length}--hfov:{hfov}---vfov:{vfov}---")
...
class Demo:
    ...
    def localize(self, image: np.ndarray, camera: Camera, canvas: Canvas, **kwargs):
        ...
        # 返回：位置(uv), 朝向(yaw), 概率(prob), 神经地图, 矫正图像
