...
def resolve_checkpoint_path(experiment_or_path: str) -> Path:
    ...
    print(f"resolve_checkpoint_path func--->path: {path}")
    if not path.exists():
        # provided name of experiment
        print(f"resolve_checkpoint_path func--->EXPERIMENTS_PATH: {EXPERIMENTS_PATH}")
    ...
    print(f"resolve_checkpoint_path func--->maybe_path: {maybe_path}")
    return maybe_path
...
def evaluate(
    ...
    **kwargs,
    ...
    print(f"evaluate func--->experiment: {experiment}")
    logger.info("Evaluating model %s with config %s", experiment, cfg)
    checkpoint_path = resolve_checkpoint_path(experiment)
    print(f"evaluate func--->output_dir0: {output_dir}")
    model = GenericModule.load_from_checkpoint(
        checkpoint_path, cfg=cfg, find_best=not experiment.endswith(".ckpt")
    )
    print(f"evaluate func--->output_dir1: {output_dir}")
    model = model.eval()
    print(f"evaluate func--->output_dir2: {output_dir}")
    if torch.cuda.is_available():
        model = model.cuda()

    dataset.prepare_data()
    dataset.setup()
    print(f"evaluate func--->output_dir: {output_dir}")
    ...
    print(f"evaluate func--->callback: {callback}")
    kwargs = {**kwargs, "callback": callback}

    seed_everything(dataset.cfg.seed)
    print(f"evaluate func--->sequential: {sequential}")
    if sequential:
